...
{
    public class ExecutanteServico : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do executante do serviço com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id do executante do serviço que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetExecutanteServicoDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          + "exse.id_executante_servico AS IdExecutanteServico, "
                                          + "exse.nome AS NomeExecutanteServico, "
                                          + "stat.id_status AS IdStatusExecutanteServico, "
                                          + "stat.nome AS NomeStatusExecutanteServico "
                                          + "FROM tb_executantes_servico AS exse "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = exse.id_status "
                                          + "WHERE exse.id_executante_servico = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdExecutanteServico"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeExecutanteServico"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusExecutanteServico"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusExecutanteServico"]);
                            }
                        }
                    }
                }
        ...
        public async Task GetExecutanteServicoDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva o executante do serviço na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarExecutanteServicoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_executante_servico", db.conexao))
                    {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_executante_servico", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_executante_servico", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
        ...
        /// <summary>
        /// Método assíncrono que deleta o executante do serviço na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarExecutanteServicoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_executante_servico", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_executante_servico", Id);
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de executantes do serviço com os argumentos utilizados
        /// </summary>
        /// <param name="listaExecutantesServico">Representa a lista de executantes do serviço que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaExecutantesServicoAsync(ObservableCollection<ExecutanteServico> listaExecutantesServico, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();

            // Verifica se a lista não foi instanciada e, caso verdadeiro, cria nova instância da lista
            if (listaExecutantesServico == null)
            {
                listaExecutantesServico = new();
            }

            // Limpa a lista caso verdadeiro
            if (limparLista)
            {
                listaExecutantesServico.Clear();
            }
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdExecutanteServico"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeExecutanteServico"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusExecutanteServico"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusExecutanteServico"]);

                                // Lança exceção de cancelamento caso ela tenha sido efetuada
                                ct.ThrowIfCancellationRequested();

                                // Adiciona o item à coleção
                                listaExecutantesServico.Add(item);
        ...
        public object Clone()
        {
            ExecutanteServico executanteServicoCopia = new();
            executanteServicoCopia.Id = Id;
            executanteServicoCopia.Nome = Nome;
            executanteServicoCopia.Status = Status == null ? new() : (Status)Status.Clone();

            return executanteServicoCopia;
        }
