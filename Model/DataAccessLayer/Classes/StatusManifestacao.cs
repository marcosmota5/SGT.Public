...
{
    public class StatusManifestacao : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do status da manifestação através do id
        /// </summary>
        /// <param name="id">Representa o id do status da manifestação que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetStatusManifestacaoDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoProreportsMySQL())
            {
                ...
                using (var command = db.conexaoProreports.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          ...
                                          + "FROM tb_status_manifestacoes AS stma "
                                          + "WHERE stma.id_status_manifestacao = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            {
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusManifestacao"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusManifestacao"]);
                            }
                        }
                    }
                }
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do status da manifestação através de condições
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task GetStatusManifestacaoDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de status de manifestação com os argumentos utilizados
        /// </summary>
        /// <param name="listaStatusManifestacao">Representa a lista de status de manifestação que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaStatusManifestacaoAsync(ObservableCollection<StatusManifestacao> listaStatusManifestacao, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();

            // Verifica se a lista não foi instanciada e, caso verdadeiro, cria nova instância da lista
            if (listaStatusManifestacao == null)
            {
                listaStatusManifestacao = new();
            }

            // Limpa a lista caso verdadeiro
            if (limparLista)
            {
                listaStatusManifestacao.Clear();
            }
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                StatusManifestacao item = new();

                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusManifestacao"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusManifestacao"]);

                                // Lança exceção de cancelamento caso ela tenha sido efetuada
                                ct.ThrowIfCancellationRequested();

                                // Adiciona o item à coleção
                                listaStatusManifestacao.Add(item);
        ...
        public object Clone()
        {
            StatusManifestacao statusManifestacaoCopia = new();
            statusManifestacaoCopia.Id = Id;
            statusManifestacaoCopia.Nome = Nome;

            return statusManifestacaoCopia;
        }
