...
{
    public class StatusAprovacao : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do status da aprovação com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id do status da aprovação que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetStatusAprovacaoDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          + "stap.id_status_aprovacao AS IdStatusAprovacao, "
                                          + "stap.nome AS NomeStatusAprovacao, "
                                          + "stat.id_status AS IdStatusStatusAprovacao, "
                                          + "stat.nome AS NomeStatusStatusAprovacao "
                                          + "FROM tb_status_aprovacao AS stap "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = stap.id_status "
                                          + "WHERE stap.id_status_aprovacao = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusAprovacao"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusAprovacao"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusStatusAprovacao"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusStatusAprovacao"]);
                            }
                        }
                    }
                }
        ...
        public async Task GetStatusAprovacaoDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva o status da aprovação na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarStatusAprovacaoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_status_aprovacao", db.conexao))
                    {
                        ...
                        {
                            throw new ValorJaExisteException("status da aprovação", Nome);
                        }
                    }
                }
                else
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_status_aprovacao", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_status_aprovacao", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
                        ...
                        {
                            throw new ValorNaoExisteException("status da aprovação", Nome);
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// Método assíncrono que deleta o status da aprovação na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarStatusAprovacaoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_status_aprovacao", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_status_aprovacao", Id);
                    ...
                    catch (MySqlConnector.MySqlException ex)
                    {
                        ...
                        {
                            throw new ChaveEstrangeiraEmUsoException("status da aprovação", Nome);
                        }
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de status de aprovação com os argumentos utilizados
        /// </summary>
        /// <param name="listaStatusAprovacao">Representa a lista de status de aprovação que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaStatusAprovacaoAsync(ObservableCollection<StatusAprovacao> listaStatusAprovacao, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();

            // Verifica se a lista não foi instanciada e, caso verdadeiro, cria nova instância da lista
            if (listaStatusAprovacao == null)
            {
                listaStatusAprovacao = new();
            }

            // Limpa a lista caso verdadeiro
            if (limparLista)
            {
                listaStatusAprovacao.Clear();
            }
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusAprovacao"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusAprovacao"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusStatusAprovacao"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusStatusAprovacao"]);

                                // Lança exceção de cancelamento caso ela tenha sido efetuada
                                ct.ThrowIfCancellationRequested();

                                // Adiciona o item à coleção
                                listaStatusAprovacao.Add(item);
        ...
        public object Clone()
        {
            StatusAprovacao statusAprovacaoCopia = new();
            statusAprovacaoCopia.Id = Id;
            statusAprovacaoCopia.Nome = Nome;
            statusAprovacaoCopia.Status = Status == null ? new() : (Status)Status.Clone();

            return statusAprovacaoCopia;
        }
