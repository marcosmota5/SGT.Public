...
{
    public class Modulo : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do modulo com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id do modulo que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetModuloDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          + "modu.id_modulo AS IdModulo, "
                                          + "modu.nome AS NomeModulo, "
                                          + "stat.id_status AS IdStatusModulo, "
                                          + "stat.nome AS NomeStatusModulo "
                                          + "FROM tb_modulos AS modu "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = modu.id_status "
                                          + "WHERE modu.id_modulo = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdModulo"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeModulo"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusModulo"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusModulo"]);
                            }
                        }
                    }
                }
        ...
        public async Task GetModuloDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva o modulo na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarModuloDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_modulo", db.conexao))
                    {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_modulo", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_modulo", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
        ...
        /// <summary>
        /// Método assíncrono que deleta o modulo na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarModuloDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_modulo", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_modulo", Id);
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de modulos com os argumentos utilizados
        /// </summary>
        /// <param name="listaModulos">Representa a lista de modulos que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaModulosAsync(ObservableCollection<Modulo> listaModulos, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdModulo"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeModulo"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusModulo"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusModulo"]);
        ...
        public object Clone()
        {
            Modulo moduloCopia = new();
            moduloCopia.Id = Id;
            moduloCopia.Nome = Nome;
            moduloCopia.Status = Status == null ? new() : (Status)Status.Clone();
