...
{
    public class Inconsistencia : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados da inconsistencia com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id da inconsistencia que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetInconsistenciaDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          + "inco.id_inconsistencia AS IdInconsistencia, "
                                          + "inco.nome AS NomeInconsistencia, "
                                          + "stat.id_status AS IdStatusInconsistencia, "
                                          + "stat.nome AS NomeStatusInconsistencia "
                                          + "FROM tb_inconsistencias AS inco "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = inco.id_status "
                                          + "WHERE inco.id_inconsistencia = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdInconsistencia"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeInconsistencia"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusInconsistencia"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusInconsistencia"]);
                            }
                        }
                    }
                }
        ...
        public async Task GetInconsistenciaDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva a inconsistencia na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarInconsistenciaDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_inconsistencia", db.conexao))
                    {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_inconsistencia", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_inconsistencia", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
        ...
        /// <summary>
        /// Método assíncrono que deleta a inconsistencia na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarInconsistenciaDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_inconsistencia", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_inconsistencia", Id);
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de inconsistencias com os argumentos utilizados
        /// </summary>
        /// <param name="listaInconsistencias">Representa a lista de inconsistencias que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaInconsistenciasAsync(ObservableCollection<Inconsistencia> listaInconsistencias, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdInconsistencia"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeInconsistencia"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusInconsistencia"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusInconsistencia"]);

                                // Lança exceção de cancelamento caso ela tenha sido efetuada
                                ct.ThrowIfCancellationRequested();

                                // Adiciona o item à coleção
                                listaInconsistencias.Add(item);
        ...
        public object Clone()
        {
            Inconsistencia inconsistenciaCopia = new();
            inconsistenciaCopia.Id = Id;
            inconsistenciaCopia.Nome = Nome;
            inconsistenciaCopia.Status = Status == null ? new() : (Status)Status.Clone();

            return inconsistenciaCopia;
        }
