...
{
    public class UsoIndevido : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do uso indevido com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id do uso indevido que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetUsoIndevidoDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          + "usoi.id_uso_indevido AS IdUsoIndevido, "
                                          + "usoi.nome AS NomeUsoIndevido, "
                                          + "stat.id_status AS IdStatusUsoIndevido, "
                                          + "stat.nome AS NomeStatusUsoIndevido "
                                          + "FROM tb_usos_indevidos AS usoi "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = usoi.id_status "
                                          + "WHERE usoi.id_uso_indevido = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdUsoIndevido"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeUsoIndevido"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusUsoIndevido"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusUsoIndevido"]);
                            }
                        }
                    }
                }
        ...
        public async Task GetUsoIndevidoDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva o uso indevido na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarUsoIndevidoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_uso_indevido", db.conexao))
                    {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_uso_indevido", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_uso_indevido", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
        ...
        /// <summary>
        /// Método assíncrono que deleta o uso indevido na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarUsoIndevidoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_uso_indevido", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_uso_indevido", Id);
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de usos indevidos com os argumentos utilizados
        /// </summary>
        /// <param name="listaUsosIndevidos">Representa a lista de usos indevidos que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaUsosIndevidosAsync(ObservableCollection<UsoIndevido> listaUsosIndevidos, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();

            // Verifica se a lista não foi instanciada e, caso verdadeiro, cria nova instância da lista
            if (listaUsosIndevidos == null)
            {
                listaUsosIndevidos = new();
            }

            // Limpa a lista caso verdadeiro
            if (limparLista)
            {
                listaUsosIndevidos.Clear();
            }
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdUsoIndevido"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeUsoIndevido"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusUsoIndevido"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusUsoIndevido"]);

                                // Lança exceção de cancelamento caso ela tenha sido efetuada
                                ct.ThrowIfCancellationRequested();

                                // Adiciona o item à coleção
                                listaUsosIndevidos.Add(item);
        ...
        public object Clone()
        {
            UsoIndevido usoIndevidoCopia = new();
            usoIndevidoCopia.Id = Id;
            usoIndevidoCopia.Nome = Nome;
            usoIndevidoCopia.Status = Status == null ? new() : (Status)Status.Clone();

            return usoIndevidoCopia;
        }
