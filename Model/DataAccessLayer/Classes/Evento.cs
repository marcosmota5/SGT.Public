...
{
    public class Evento : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do evento com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id do evento que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetEventoDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          + "even.id_evento AS IdEvento, "
                                          + "even.nome AS NomeEvento, "
                                          + "stat.id_status AS IdStatusEvento, "
                                          + "stat.nome AS NomeStatusEvento "
                                          + "FROM tb_eventos AS even "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = even.id_status "
                                          + "WHERE even.id_evento = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdEvento"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeEvento"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusEvento"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusEvento"]);
                            }
                        }
                    }
                }
        ...
        public async Task GetEventoDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva o evento na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarEventoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_evento", db.conexao))
                    {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_evento", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_evento", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
        ...
        /// <summary>
        /// Método assíncrono que deleta o evento na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarEventoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_evento", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_evento", Id);
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de eventos com os argumentos utilizados
        /// </summary>
        /// <param name="listaEventos">Representa a lista de eventos que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaEventosAsync(ObservableCollection<Evento> listaEventos, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdEvento"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeEvento"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusEvento"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusEvento"]);
        ...
        public object Clone()
        {
            Evento eventoCopia = new();
            eventoCopia.Id = Id;
            eventoCopia.Nome = Nome;
            eventoCopia.Status = Status == null ? new() : (Status)Status.Clone();
