...
{
    public class Prioridade : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados da prioridade com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id da origem que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetPrioridadeDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          + "prio.id_prioridade AS IdPrioridade, "
                                          + "prio.nome AS NomePrioridade, "
                                          + "stat.id_status AS IdStatusPrioridade, "
                                          + "stat.nome AS NomeStatusPrioridade "
                                          + "FROM tb_prioridades AS prio "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = prio.id_status "
                                          + "WHERE prio.id_prioridade = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdPrioridade"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomePrioridade"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusPrioridade"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusPrioridade"]);
                            }
                        }
                    }
                }
        ...
        public async Task GetPrioridadeDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva a prioridade na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarPrioridadeDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_prioridade", db.conexao))
                    {
                        ...
                        {
                            throw new ValorJaExisteException(nameof(Prioridade).ToLower(), Nome);
                        }
                    }
                }
                else
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_prioridade", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_prioridade", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
                        ...
                        {
                            throw new ValorNaoExisteException(nameof(Prioridade).ToLower(), Nome);
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// Método assíncrono que deleta a prioridade na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarPrioridadeDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_prioridade", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_prioridade", Id);
                    ...
                    catch (MySqlConnector.MySqlException ex)
                    {
                        ...
                        {
                            throw new ChaveEstrangeiraEmUsoException(nameof(Prioridade).ToLower(), Nome);
                        }
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de prioridades com os argumentos utilizados
        /// </summary>
        /// <param name="listaPrioridades">Representa a lista de prioridades que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaPrioridadesAsync(ObservableCollection<Prioridade> listaPrioridades, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdPrioridade"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomePrioridade"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusPrioridade"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusPrioridade"]);

                                // Lança exceção de cancelamento caso ela tenha sido efetuada
                                ct.ThrowIfCancellationRequested();

                                // Adiciona o item à coleção
                                listaPrioridades.Add(item);
        ...
        public object Clone()
        {
            Prioridade prioridadeCopia = new();
            prioridadeCopia.Id = Id;
            prioridadeCopia.Nome = Nome;
            prioridadeCopia.Status = Status == null ? new() : (Status)Status.Clone();

            return prioridadeCopia;
        }
