...
{
    public class PrioridadeManifestacao : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados da prioridade da manifestação através do id
        /// </summary>
        /// <param name="id">Representa o id da prioridade da manifestação que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetPrioridadeManifestacaoDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoProreportsMySQL())
            {
                ...
                using (var command = db.conexaoProreports.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          ...
                                          + "FROM tb_prioridades_manifestacoes AS prma "
                                          + "WHERE prma.id_prioridade_manifestacao = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            {
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdPrioridadeManifestacao"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomePrioridadeManifestacao"]);
                            }
                        }
                    }
                }
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados da prioridade da manifestação através de condições
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task GetPrioridadeManifestacaoDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de prioridades de manifestação com os argumentos utilizados
        /// </summary>
        /// <param name="listaPrioridadesManifestacao">Representa a lista de prioridades de manifestação que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaPrioridadesManifestacaoAsync(ObservableCollection<PrioridadeManifestacao> listaPrioridadesManifestacao, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();

            // Verifica se a lista não foi instanciada e, caso verdadeiro, cria nova instância da lista
            if (listaPrioridadesManifestacao == null)
            {
                listaPrioridadesManifestacao = new();
            }

            // Limpa a lista caso verdadeiro
            if (limparLista)
            {
                listaPrioridadesManifestacao.Clear();
            }
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                PrioridadeManifestacao item = new();

                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdPrioridadeManifestacao"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomePrioridadeManifestacao"]);

                                // Lança exceção de cancelamento caso ela tenha sido efetuada
                                ct.ThrowIfCancellationRequested();

                                // Adiciona o item à coleção
                                listaPrioridadesManifestacao.Add(item);
        ...
        public object Clone()
        {
            PrioridadeManifestacao prioridadeManifestacaoCopia = new();
            prioridadeManifestacaoCopia.Id = Id;
            prioridadeManifestacaoCopia.Nome = Nome;

            return prioridadeManifestacaoCopia;
        }
