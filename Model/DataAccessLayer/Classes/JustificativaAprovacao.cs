...
{
    public class JustificativaAprovacao : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados da justificativa da aprovação com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id da justificativa da aprovação que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetJustificativaAprovacaoDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          + "juap.id_justificativa_aprovacao AS IdJustificativaAprovacao, "
                                          + "juap.nome AS NomeJustificativaAprovacao, "
                                          + "stat.id_status AS IdStatusJustificativaAprovacao, "
                                          + "stat.nome AS NomeStatusJustificativaAprovacao "
                                          + "FROM tb_justificativas_aprovacao AS juap "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = juap.id_status "
                                          + "WHERE juap.id_justificativa_aprovacao = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdJustificativaAprovacao"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeJustificativaAprovacao"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusJustificativaAprovacao"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusJustificativaAprovacao"]);
                            }
                        }
                    }
                }
        ...
        public async Task GetJustificativaAprovacaoDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva a justificativa da aprovação na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarJustificativaAprovacaoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_justificativa_aprovacao", db.conexao))
                    {
                        ...
                        {
                            throw new ValorJaExisteException("justificativa da aprovação", Nome);
                        }
                    }
                }
                else
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_justificativa_aprovacao", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_justificativa_aprovacao", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
                        ...
                        {
                            throw new ValorNaoExisteException("justificativa da aprovação", Nome);
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// Método assíncrono que deleta a justificativa da aprovação na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarJustificativaAprovacaoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_justificativa_aprovacao", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_justificativa_aprovacao", Id);
                    ...
                    catch (MySqlConnector.MySqlException ex)
                    {
                        ...
                        {
                            throw new ChaveEstrangeiraEmUsoException("justificativa da aprovação", Nome);
                        }
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de justificativas de aprovação com os argumentos utilizados
        /// </summary>
        /// <param name="listaJustificativasAprovacao">Representa a lista de justificativas de aprovação que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaJustificativasAprovacaoAsync(ObservableCollection<JustificativaAprovacao> listaJustificativasAprovacao, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();

            // Verifica se a lista não foi instanciada e, caso verdadeiro, cria nova instância da lista
            if (listaJustificativasAprovacao == null)
            {
                listaJustificativasAprovacao = new();
            }

            // Limpa a lista caso verdadeiro
            if (limparLista)
            {
                listaJustificativasAprovacao.Clear();
            }
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                JustificativaAprovacao item = new();
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdJustificativaAprovacao"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeJustificativaAprovacao"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusJustificativaAprovacao"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusJustificativaAprovacao"]);

                                // Lança exceção de cancelamento caso ela tenha sido efetuada
                                ct.ThrowIfCancellationRequested();

                                // Adiciona o item à coleção
                                listaJustificativasAprovacao.Add(item);
        ...
        public object Clone()
        {
            JustificativaAprovacao justificativaAprovacaoCopia = new();
            justificativaAprovacaoCopia.Id = Id;
            justificativaAprovacaoCopia.Nome = Nome;
            justificativaAprovacaoCopia.Status = Status == null ? new() : (Status)Status.Clone();

            return justificativaAprovacaoCopia;
        }
