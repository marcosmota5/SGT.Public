...
{
    public class Classe : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do classe com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id do classe que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetClasseDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          ...
                                          + "stat.id_status AS IdStatusClasse, "
                                          + "stat.nome AS NomeStatusClasse "
                                          + "FROM tb_classes AS clas "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = clas.id_status "
                                          + "WHERE clas.id_classe = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdClasse"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeClasse"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusClasse"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusClasse"]);
                            }
                        }
                    }
                }
        ...
        public async Task GetClasseDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva o classe na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarClasseDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_classe", db.conexao))
                    {
                        ...
                        {
                            throw new ValorJaExisteException(nameof(Classe).ToLower(), Nome);
                        }
                    }
                }
                else
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_classe", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_classe", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
                        ...
                        {
                            throw new ValorNaoExisteException(nameof(Classe).ToLower(), Nome);
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// Método assíncrono que deleta o classe na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarClasseDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_classe", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_classe", Id);
                    ...
                    catch (MySqlConnector.MySqlException ex)
                    {
                        ...
                        {
                            throw new ChaveEstrangeiraEmUsoException(nameof(Classe).ToLower(), Nome);
                        }
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de classes com os argumentos utilizados
        /// </summary>
        /// <param name="listaClasses">Representa a lista de classes que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaClassesAsync(ObservableCollection<Classe> listaClasses, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdClasse"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeClasse"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusClasse"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusClasse"]);
        ...
        public object Clone()
        {
            Classe classeCopia = new();
            classeCopia.Id = Id;
            classeCopia.Nome = Nome;
            classeCopia.Status = Status == null ? new() : (Status)Status.Clone();
