...
{
    public class TipoItem : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do tipo do item com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id do tipo do item que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetTipoItemDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          + "tiit.id_tipo_item AS IdTipoItem, "
                                          + "tiit.nome AS NomeTipoItem, "
                                          + "stat.id_status AS IdStatusTipoItem, "
                                          + "stat.nome AS NomeStatusTipoItem "
                                          + "FROM tb_tipos_item AS tiit "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = tiit.id_status "
                                          + "WHERE tiit.id_tipo_item = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdTipoItem"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeTipoItem"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusTipoItem"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusTipoItem"]);
                            }
                        }
                    }
                }
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do tipo de item com os argumentos utilizados
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task GetTipoItemDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva o tipo do item na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarTipoItemDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_tipo_item", db.conexao))
                    {
                        ...
                        {
                            throw new ValorJaExisteException("tipo do item", Nome);
                        }
                    }
                }
                else
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_tipo_item", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_tipo_item", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
                        ...
                        {
                            throw new ValorNaoExisteException("tipo do item", Nome);
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// Método assíncrono que deleta o tipo do item na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarTipoItemDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_tipo_item", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_tipo_item", Id);
                    ...
                    catch (MySqlConnector.MySqlException ex)
                    {
                        ...
                        {
                            throw new ChaveEstrangeiraEmUsoException("tipo do item", Nome);
                        }
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de tipos de item com os argumentos utilizados
        /// </summary>
        /// <param name="listaTiposItem">Representa a lista de tipos de item que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaTiposItemAsync(ObservableCollection<TipoItem> listaTiposItem, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();

            // Verifica se a lista não foi instanciada e, caso verdadeiro, cria nova instância da lista
            if (listaTiposItem == null)
            {
                listaTiposItem = new();
            }
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdTipoItem"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeTipoItem"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusTipoItem"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusTipoItem"]);

                                // Lança exceção de cancelamento caso ela tenha sido efetuada
                                ct.ThrowIfCancellationRequested();

                                // Adiciona o item à coleção
                                listaTiposItem.Add(item);
        ...
        public object Clone()
        {
            TipoItem tipoItemCopia = new();
            tipoItemCopia.Id = Id;
            tipoItemCopia.Nome = Nome;
            tipoItemCopia.Status = Status == null ? new() : (Status)Status.Clone();

            return tipoItemCopia;
        }
