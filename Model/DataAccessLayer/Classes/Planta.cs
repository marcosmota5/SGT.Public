...
{
    public class Planta : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do planta com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id do planta que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetPlantaDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          ...
                                          + "stat.id_status AS IdStatusPlanta, "
                                          + "stat.nome AS NomeStatusPlanta, "
                                          ...
                                          + "FROM tb_plantas AS plan "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = plan.id_status "
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdPlanta"]);
                                IdRetornado = FuncoesDeConversao.ConverteParaInt(reader["IdPlanta"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomePlanta"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusPlanta"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusPlanta"]);
        ...
        public async Task GetPlantaDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva o planta na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarPlantaDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_planta", db.conexao))
                    {
                        ...
                        command.Parameters.Add("p_id_planta", MySqlConnector.MySqlDbType.Int32).Direction = System.Data.ParameterDirection.Output;
                        ...
                        {
                            throw new ValorJaExisteException("O planta " + Nome + " já existe para o cliente " + Cliente.Nome);
                        }

                        // Retorna o id do planta
                        Id = FuncoesDeConversao.ConverteParaInt(command.Parameters["p_id_planta"].Value);
                    }
                }
                else
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_planta", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_planta", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_pais", Cidade.Estado.Pais.Id);
                        command.Parameters.AddWithValue("p_id_estado", Cidade.Estado.Id);
                        command.Parameters.AddWithValue("p_id_cidade", Cidade.Id);
                        ...
                        {
                            throw new ValorNaoExisteException(nameof(Planta).ToLower(), Nome);
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// Método assíncrono que deleta o planta na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarPlantaDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_planta", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_planta", Id);
                    ...
                    catch (MySqlConnector.MySqlException ex)
                    {
                        ...
                        {
                            throw new ChaveEstrangeiraEmUsoException(nameof(Planta).ToLower(), Nome);
                        }
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de plantas com os argumentos utilizados
        /// </summary>
        /// <param name="listaPlantas">Representa a lista de plantas que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaPlantasAsync(ObservableCollection<Planta> listaPlantas, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdPlanta"]);
                                item.IdRetornado = FuncoesDeConversao.ConverteParaInt(reader["IdPlanta"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomePlanta"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusPlanta"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusPlanta"]);
        ...
        public object Clone()
        {
            Planta plantaCopia = new();

            plantaCopia.Id = Id;
            plantaCopia.Nome = Nome;
            plantaCopia.Status = Status == null ? new() : (Status)Status.Clone();
            plantaCopia.Cliente = Cliente == null ? new() : (Cliente)Cliente.Clone();
            plantaCopia.Cidade = Cidade == null ? new() : (Cidade)Cidade.Clone();
