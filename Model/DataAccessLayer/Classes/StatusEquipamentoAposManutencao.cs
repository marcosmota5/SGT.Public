...
{
    public class StatusEquipamentoAposManutencao : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// </summary>
        public StatusEquipamentoAposManutencao()
        {
            Status = new();
        }
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do status do equipamento após a manutenção com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id do status do equipamento após a manutenção que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetStatusEquipamentoAposManutencaoDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          + "seam.id_status_equipamento_apos_manutencao AS IdStatusEquipamentoAposManutencao, "
                                          + "seam.nome AS NomeStatusEquipamentoAposManutencao, "
                                          + "stat.id_status AS IdStatusStatusEquipamentoAposManutencao, "
                                          + "stat.nome AS NomeStatusStatusEquipamentoAposManutencao "
                                          + "FROM tb_status_equipamento_apos_manutencao AS seam "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = seam.id_status "
                                          + "WHERE seam.id_status_equipamento_apos_manutencao = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusEquipamentoAposManutencao"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusEquipamentoAposManutencao"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusStatusEquipamentoAposManutencao"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusStatusEquipamentoAposManutencao"]);
                            }
                        }
                    }
                }
        ...
        public async Task GetStatusEquipamentoAposManutencaoDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva o status do equipamento após a manutenção na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarStatusEquipamentoAposManutencaoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_status_equipamento_apos_manutencao", db.conexao))
                    {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_status_equipamento_apos_manutencao", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_status_equipamento_apos_manutencao", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
        ...
        /// <summary>
        /// Método assíncrono que deleta o status do equipamento após a manutenção na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarStatusEquipamentoAposManutencaoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_status_equipamento_apos_manutencao", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_status_equipamento_apos_manutencao", Id);
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de status do equipamento após a manutenção com os argumentos utilizados
        /// </summary>
        /// <param name="listaStatusEquipamentoAposManutencao">Representa a lista de status do equipamento após a manutenção que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaStatusEquipamentoAposManutencaoAsync(ObservableCollection<StatusEquipamentoAposManutencao> listaStatusEquipamentoAposManutencao, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();

            // Verifica se a lista não foi instanciada e, caso verdadeiro, cria nova instância da lista
            if (listaStatusEquipamentoAposManutencao == null)
            {
                listaStatusEquipamentoAposManutencao = new();
            }

            // Limpa a lista caso verdadeiro
            if (limparLista)
            {
                listaStatusEquipamentoAposManutencao.Clear();
            }
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                StatusEquipamentoAposManutencao item = new();
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusEquipamentoAposManutencao"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusEquipamentoAposManutencao"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusStatusEquipamentoAposManutencao"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusStatusEquipamentoAposManutencao"]);

                                // Lança exceção de cancelamento caso ela tenha sido efetuada
                                ct.ThrowIfCancellationRequested();

                                // Adiciona o item à coleção
                                listaStatusEquipamentoAposManutencao.Add(item);
        ...
        public object Clone()
        {
            StatusEquipamentoAposManutencao statusEquipamentoAposManutencaoCopia = new();
            statusEquipamentoAposManutencaoCopia.Id = Id;
            statusEquipamentoAposManutencaoCopia.Nome = Nome;
            statusEquipamentoAposManutencaoCopia.Status = Status == null ? new() : (Status)Status.Clone();

            return statusEquipamentoAposManutencaoCopia;
        }
