...
{
    public class Setor : ObservableObject, ICloneable
    {
        #region Campos

        private int? _id;
        private string? _nome;
        private int? _prazoAdicional;
        ...
        {
            get { return _prazoAdicional; } 
            set
            {
                if(value != _prazoAdicional)
                {
                    _prazoAdicional = value;
                    OnPropertyChanged(nameof(PrazoAdicional));
                }
            }
        }
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do setor com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id do setor que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetSetorDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          ...
                                          + "stat.id_status AS IdStatusSetor, "
                                          + "stat.nome AS NomeStatusSetor "
                                          + "FROM tb_setores AS seto "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = seto.id_status "
                                          + "WHERE seto.id_setor = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdSetor"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeSetor"]);
                                PrazoAdicional = FuncoesDeConversao.ConverteParaInt(reader["PrazoAdicional"]);
        ...
        public async Task GetSetorDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva o setor na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarSetorDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_setor", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_prazo_adicional", PrazoAdicional);
                        ...
                        {
                            throw new ValorJaExisteException(nameof(Setor).ToLower(), Nome);
                        }
                    }
                }
                else
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_setor", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_setor", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_prazo_adicional", PrazoAdicional);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
                        ...
                        {
                            throw new ValorNaoExisteException(nameof(Setor).ToLower(), Nome);
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// Método assíncrono que deleta o setor na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarSetorDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_setor", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_setor", Id);
                    ...
                    catch (MySqlConnector.MySqlException ex)
                    {
                        ...
                        {
                            throw new ChaveEstrangeiraEmUsoException(nameof(Setor).ToLower(), Nome);
                        }
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de setores com os argumentos utilizados
        /// </summary>
        /// <param name="listaSetores">Representa a lista de setores que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaSetoresAsync(ObservableCollection<Setor> listaSetores, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdSetor"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeSetor"]);
                                item.PrazoAdicional = FuncoesDeConversao.ConverteParaInt(reader["PrazoAdicional"]);
        ...
        public object Clone()
        {
            Setor setorCopia = new();
            setorCopia.Id = Id;
            setorCopia.Nome = Nome;
            setorCopia.PrazoAdicional = PrazoAdicional;
            setorCopia.Status = Status == null ? new() : (Status)Status.Clone();
