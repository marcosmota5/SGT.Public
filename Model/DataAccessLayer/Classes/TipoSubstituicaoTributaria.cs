...
{
    public class TipoSubstituicaoTributaria : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// </summary>
        public TipoSubstituicaoTributaria()
        {
            Status = new();
        }
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do tipo de substituicao tributaria com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id do tipo de substituicao tributaria que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetTipoSubstituicaoTributariaDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          + "tisu.id_tipo_substituicao_tributaria AS IdTipoSubstituicaoTributaria, "
                                          + "tisu.nome AS NomeTipoSubstituicaoTributaria, "
                                          + "stat.id_status AS IdStatusTipoSubstituicaoTributaria, "
                                          + "stat.nome AS NomeStatusTipoSubstituicaoTributaria "
                                          + "FROM tb_tipos_substituicao_tributaria AS tisu "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = tisu.id_status "
                                          + "WHERE tisu.id_tipo_substituicao_tributaria = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdTipoSubstituicaoTributaria"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeTipoSubstituicaoTributaria"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusTipoSubstituicaoTributaria"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusTipoSubstituicaoTributaria"]);
                            }
                        }
                    }
                }
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do tipo de substituição tributária com os argumentos utilizados
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task GetTipoSubstituicaoTributariaDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva o tipo de substituição tributária na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarTipoSubstituicaoTributariaDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_tipo_substituicao_tributaria", db.conexao))
                    {
                        ...
                        {
                            throw new ValorJaExisteException("tipo de substituição tributária", Nome);
                        }
                    }
                }
                else
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_tipo_substituicao_tributaria", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_tipo_substituicao_tributaria", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
                        ...
                        {
                            throw new ValorNaoExisteException("tipo de substituição tributária", Nome);
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// Método assíncrono que deleta o tipo de substituição tributária na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarTipoSubstituicaoTributariaDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_tipo_substituicao_tributaria", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_tipo_substituicao_tributaria", Id);
                    ...
                    catch (MySqlConnector.MySqlException ex)
                    {
                        ...
                        {
                            throw new ChaveEstrangeiraEmUsoException("tipo de substituição tributária", Nome);
                        }
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de tipos de substituição tributária com os argumentos utilizados
        /// </summary>
        /// <param name="listaTiposSubstituicaoTributaria">Representa a lista de tipos de substituição tributária que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaSubstituicaoTributariaItemAsync(ObservableCollection<TipoSubstituicaoTributaria> listaTiposSubstituicaoTributaria, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();

            // Verifica se a lista não foi instanciada e, caso verdadeiro, cria nova instância da lista
            if (listaTiposSubstituicaoTributaria == null)
            {
                listaTiposSubstituicaoTributaria = new();
            }

            // Limpa a lista caso verdadeiro
            if (limparLista)
            {
                listaTiposSubstituicaoTributaria.Clear();
            }
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                TipoSubstituicaoTributaria item = new();
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdTipoSubstituicaoTributaria"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeTipoSubstituicaoTributaria"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusTipoSubstituicaoTributaria"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusTipoSubstituicaoTributaria"]);

                                // Lança exceção de cancelamento caso ela tenha sido efetuada
                                ct.ThrowIfCancellationRequested();

                                // Adiciona o item à coleção
                                listaTiposSubstituicaoTributaria.Add(item);
        ...
        public object Clone()
        {
            TipoSubstituicaoTributaria tipoSubstituicaoTributariaCopia = new();
            tipoSubstituicaoTributariaCopia.Id = Id;
            tipoSubstituicaoTributariaCopia.Nome = Nome;
            tipoSubstituicaoTributariaCopia.Status = Status == null ? new() : (Status)Status.Clone();

            return tipoSubstituicaoTributariaCopia;
        }
