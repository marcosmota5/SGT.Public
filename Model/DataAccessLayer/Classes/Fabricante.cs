...
{
    public class Fabricante : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do fabricante com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id do fabricante que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetFabricanteDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          ...
                                          + "stat.id_status AS IdStatusFabricante, "
                                          + "stat.nome AS NomeStatusFabricante "
                                          + "FROM tb_fabricantes AS fabr "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = fabr.id_status "
                                          + "WHERE fabr.id_fabricante = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdFabricante"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeFabricante"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusFabricante"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusFabricante"]);
                            }
                        }
                    }
                }
        ...
        public async Task GetFabricanteDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva o conjunto na fabricante, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarFabricanteDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_fabricante", db.conexao))
                    {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_fabricante", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_fabricante", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
        ...
        /// <summary>
        /// Método assíncrono que deleta o fabricante na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarFabricanteDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_fabricante", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_fabricante", Id);
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de fabricantes com os argumentos utilizados
        /// </summary>
        /// <param name="listaFabricantes">Representa a lista de fabricantes que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaFabricantesAsync(ObservableCollection<Fabricante> listaFabricantes, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdFabricante"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeFabricante"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusFabricante"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusFabricante"]);
        ...
        public object Clone()
        {
            Fabricante fabricanteCopia = new();
            fabricanteCopia.Id = Id;
            fabricanteCopia.Nome = Nome;
            fabricanteCopia.Status = Status == null ? new() : (Status)Status.Clone();

            return fabricanteCopia;
        }
