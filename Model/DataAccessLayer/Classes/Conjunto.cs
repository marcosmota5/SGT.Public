...
{
    public class Conjunto : ObservableObject, ICloneable
    {
        #region Campos
        ...
        private ObservableCollection<Especificacao> _listaEspecificacoes;
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do conjunto com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id do conjunto que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetConjuntoDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          ...
                                          + "stat.id_status AS IdStatusConjunto, "
                                          + "stat.nome AS NomeStatusConjunto "
                                          + "FROM tb_conjuntos AS conj "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = conj.id_status "
                                          + "WHERE conj.id_conjunto = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdConjunto"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeConjunto"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusConjunto"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusConjunto"]);
                            }
                        }
                    }
                }
        ...
        public async Task GetConjuntoDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva o conjunto na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarConjuntoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_conjunto", db.conexao))
                    {
                        ...
                        {
                            throw new ValorJaExisteException(nameof(Conjunto).ToLower(), Nome);
                        }
                    }
                }
                else
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_conjunto", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_conjunto", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
                        ...
                        {
                            throw new ValorNaoExisteException(nameof(Conjunto).ToLower(), Nome);
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// Método assíncrono que deleta o conjunto na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarConjuntoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_conjunto", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_conjunto", Id);
                    ...
                    catch (MySqlConnector.MySqlException ex)
                    {
                        ...
                        {
                            throw new ChaveEstrangeiraEmUsoException(nameof(Conjunto).ToLower(), Nome);
                        }
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de conjuntos com os argumentos utilizados
        /// </summary>
        /// <param name="listaConjuntos">Representa a lista de conjuntos que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaConjuntosAsync(ObservableCollection<Conjunto> listaConjuntos, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdConjunto"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeConjunto"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusConjunto"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusConjunto"]);

                                // Lança exceção de cancelamento caso ela tenha sido efetuada
                                ct.ThrowIfCancellationRequested();

                                // Adiciona o item à coleção
                                listaConjuntos.Add(item);
        ...
        public object Clone()
        {
            Conjunto conjuntoCopia = new();
            conjuntoCopia.Id = Id;
            conjuntoCopia.Nome = Nome;
            conjuntoCopia.Status = Status == null ? new() : (Status)Status.Clone();
