...
{
    public class FornecimentoItemOrdemServico : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// </summary>
        public FornecimentoItemOrdemServico()
        {
            Status = new();
        }
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do fornecimento do item da ordem de serviço com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id do fornecimento do item da ordem de serviço que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetFornecimentoItemOrdemServicoDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          + "fios.id_fornecimento_item_ordem_servico AS IdFornecimentoItemOrdemServico, "
                                          + "fios.sigla AS SiglaFornecimentoItemOrdemServico, "
                                          + "fios.nome AS NomeFornecimentoItemOrdemServico, "
                                          + "stat.id_status AS IdStatusFornecimentoItemOrdemServico, "
                                          + "stat.nome AS NomeStatusFornecimentoItemOrdemServico "
                                          + "FROM tb_fornecimentos_item_ordem_servico AS fios "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = fios.id_status "
                                          + "WHERE fios.id_fornecimento_item_ordem_servico = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdFornecimentoItemOrdemServico"]);
                                Sigla = FuncoesDeConversao.ConverteParaString(reader["SiglaFornecimentoItemOrdemServico"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeFornecimentoItemOrdemServico"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusFornecimentoItemOrdemServico"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusFornecimentoItemOrdemServico"]);
                            }
                        }
                    }
                }
        ...
        public async Task GetFornecimentoItemOrdemServicoDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva o fornecimento do item da ordem de serviço na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarFornecimentoItemOrdemServicoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_fornecimento_item_ordem_servico", db.conexao))
                    {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_fornecimento_item_ordem_servico", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_fornecimento_item_ordem_servico", Id);
                        command.Parameters.AddWithValue("p_sigla", Sigla);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
        ...
        /// <summary>
        /// Método assíncrono que deleta o fornecimento do item da ordem de serviço na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarFornecimentoItemOrdemServicoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_fornecimento_item_ordem_servico", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_fornecimento_item_ordem_servico", Id);
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de fornecimentos do item da ordem de serviço com os argumentos utilizados
        /// </summary>
        /// <param name="listaFornecimentosItemOrdemServico">Representa a lista de fornecimentos do item da ordem de serviço que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaFornecimentosItemOrdemServicoAsync(ObservableCollection<FornecimentoItemOrdemServico> listaFornecimentosItemOrdemServico, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();

            // Verifica se a lista não foi instanciada e, caso verdadeiro, cria nova instância da lista
            if (listaFornecimentosItemOrdemServico == null)
            {
                listaFornecimentosItemOrdemServico = new();
            }

            // Limpa a lista caso verdadeiro
            if (limparLista)
            {
                listaFornecimentosItemOrdemServico.Clear();
            }
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                FornecimentoItemOrdemServico item = new();
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdFornecimentoItemOrdemServico"]);
                                item.Sigla = FuncoesDeConversao.ConverteParaString(reader["SiglaFornecimentoItemOrdemServico"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeFornecimentoItemOrdemServico"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusFornecimentoItemOrdemServico"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusFornecimentoItemOrdemServico"]);

                                // Lança exceção de cancelamento caso ela tenha sido efetuada
                                ct.ThrowIfCancellationRequested();

                                // Adiciona o item à coleção
                                listaFornecimentosItemOrdemServico.Add(item);
        ...
        public object Clone()
        {
            FornecimentoItemOrdemServico fornecimentoItemOrdemServicoCopia = new();
            fornecimentoItemOrdemServicoCopia.Id = Id;
            fornecimentoItemOrdemServicoCopia.Sigla = Sigla;
            fornecimentoItemOrdemServicoCopia.Nome = Nome;
            fornecimentoItemOrdemServicoCopia.Status = Status == null ? new() : (Status)Status.Clone();

            return fornecimentoItemOrdemServicoCopia;
        }
