...
{
    public class TipoEquipamento : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do tipo de equipamento com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id do tipo de equipamento que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetTipoEquipamentoDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          ...
                                          + "stat.id_status AS IdStatusTipoEquipamento, "
                                          + "stat.nome AS NomeStatusTipoEquipamento "
                                          + "FROM tb_tipos_equipamento AS tieq "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = tieq.id_status "
                                          + "WHERE tieq.id_tipo_equipamento = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdTipoEquipamento"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeTipoEquipamento"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusTipoEquipamento"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusTipoEquipamento"]);
                            }
                        }
                    }
                }
        ...
        public async Task GetTipoEquipamentoDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva o tipo de equipamento na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarTipoEquipamentoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_tipo_equipamento", db.conexao))
                    {
                        ...
                        {
                            throw new ValorJaExisteException("tipo de equipamento", Nome);
                        }
                    }
                }
                else
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_tipo_equipamento", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_tipo_equipamento", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
                        ...
                        {
                            throw new ValorNaoExisteException("tipo de equipamento", Nome);
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// Método assíncrono que deleta o tipo de equipamento na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarTipoEquipamentoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_tipo_equipamento", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_tipo_equipamento", Id);
                    ...
                    catch (MySqlConnector.MySqlException ex)
                    {
                        ...
                        {
                            throw new ChaveEstrangeiraEmUsoException("tipo de equipamento", Nome);
                        }
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de tipos de equipamento com os argumentos utilizados
        /// </summary>
        /// <param name="listaTiposEquipamento">Representa a lista de tipos de equipamento que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaTiposEquipamentoAsync(ObservableCollection<TipoEquipamento> listaTiposEquipamento, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();

            // Verifica se a lista não foi instanciada e, caso verdadeiro, cria nova instância da lista
            if (listaTiposEquipamento == null)
            {
                listaTiposEquipamento = new();
            }

            // Limpa a lista caso verdadeiro
            if (limparLista)
            {
                listaTiposEquipamento.Clear();
            }
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdTipoEquipamento"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeTipoEquipamento"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusTipoEquipamento"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusTipoEquipamento"]);

                                // Lança exceção de cancelamento caso ela tenha sido efetuada
                                ct.ThrowIfCancellationRequested();

                                // Adiciona o item à coleção
                                listaTiposEquipamento.Add(item);
        ...
        public object Clone()
        {
            TipoEquipamento tipoEquipamentoCopia = new();
            tipoEquipamentoCopia.Id = Id;
            tipoEquipamentoCopia.Nome = Nome;
            tipoEquipamentoCopia.Status = Status == null ? new() : (Status)Status.Clone();

            return tipoEquipamentoCopia;
        }
