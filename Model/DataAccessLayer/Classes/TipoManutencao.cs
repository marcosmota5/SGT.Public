...
{
    public class TipoManutencao : ObservableObject, ICloneable
    {
        #region Campos
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados do tipo da manutenção com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id do tipo da manutenção que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetTipoManutencaoDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          + "tima.id_tipo_manutencao AS IdTipoManutencao, "
                                          + "tima.nome AS NomeTipoManutencao, "
                                          + "stat.id_status AS IdStatusTipoManutencao, "
                                          + "stat.nome AS NomeStatusTipoManutencao "
                                          + "FROM tb_tipos_manutencao AS tima "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = tima.id_status "
                                          + "WHERE tima.id_tipo_manutencao = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdTipoManutencao"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeTipoManutencao"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusTipoManutencao"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusTipoManutencao"]);
                            }
                        }
                    }
                }
        ...
        public async Task GetTipoManutencaoDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva o tipo da manutenção na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarTipoManutencaoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_tipo_manutencao", db.conexao))
                    {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_tipo_manutencao", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_tipo_manutencao", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
        ...
        /// <summary>
        /// Método assíncrono que deleta o tipo da manutenção na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarTipoManutencaoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_tipo_manutencao", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_tipo_manutencao", Id);
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de tipos de manutenção com os argumentos utilizados
        /// </summary>
        /// <param name="listaTiposManutencao">Representa a lista de tipos de manutenção que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaTiposManutencaoAsync(ObservableCollection<TipoManutencao> listaTiposManutencao, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            {
                listaTiposManutencao = new();
            }

            // Limpa a lista caso verdadeiro
            if (limparLista)
            {
                listaTiposManutencao.Clear();
            }
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdTipoManutencao"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeTipoManutencao"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusTipoManutencao"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusTipoManutencao"]);

                                // Lança exceção de cancelamento caso ela tenha sido efetuada
                                ct.ThrowIfCancellationRequested();

                                // Adiciona o item à coleção
                                listaTiposManutencao.Add(item);
        ...
        public object Clone()
        {
            TipoManutencao tipoManutencaoCopia = new();
            tipoManutencaoCopia.Id = Id;
            tipoManutencaoCopia.Nome = Nome;
            tipoManutencaoCopia.Status = Status == null ? new() : (Status)Status.Clone();

            return tipoManutencaoCopia;
        }
