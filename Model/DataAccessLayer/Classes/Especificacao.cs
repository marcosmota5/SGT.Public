...
{
    public class Especificacao : ObservableObject, ICloneable
    {
        ...
        /// <summary>
        /// Método assíncrono que retorna os dados da especificação com os argumentos utilizados
        /// </summary>
        /// <param name="id">Representa o id da especificação que deseja retornar</param>
        /// <param name="ct">Token de cancelamento</param>
        public async Task GetEspecificacaoDatabaseAsync(int? id, CancellationToken ct)
        {
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                using (var command = db.conexao.CreateCommand())
                {
                    ...
                    command.CommandText = "SELECT "
                                          + "espe.id_especificacao AS IdEspecificacao, "
                                          + "espe.nome AS NomeEspecificacao, "
                                          + "stat.id_status AS IdStatusEspecificacao, "
                                          + "stat.nome AS NomeStatusEspecificacao, "
                                          + "conj.id_conjunto AS IdConjunto, "
                                          + "conj.nome AS NomeConjunto, "
                                          + "stat_conj.id_status AS IdStatusConjunto, "
                                          + "stat_conj.nome AS NomeStatusConjunto "
                                          + "FROM tb_especificacoes AS espe "
                                          + "LEFT JOIN tb_status AS stat ON stat.id_status = espe.id_status "
                                          + "LEFT JOIN tb_conjuntos AS conj ON conj.id_conjunto = conj.id_conjunto "
                                          + "LEFT JOIN tb_status AS stat_conj ON stat_conj.id_status = conj.id_status "
                                          + "WHERE espe.id_especificacao = @id";
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                // Verifica se a classe não foi instânciada e, caso verdadeiro, cria nova instância da classe
                                if (Conjunto.Status == null)
                                {
                                    Conjunto.Status = new();
                                }

                                // Define as propriedades
                                Id = FuncoesDeConversao.ConverteParaInt(reader["IdEspecificacao"]);
                                Nome = FuncoesDeConversao.ConverteParaString(reader["NomeEspecificacao"]);
                                Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusEspecificacao"]);
                                Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusEspecificacao"]);
                                Conjunto.Id = FuncoesDeConversao.ConverteParaInt(reader["IdConjunto"]);
                                Conjunto.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeConjunto"]);
                                Conjunto.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusConjunto"]);
                                Conjunto.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusConjunto"]);
                            }
                        }
                    }
                }
        ...
        public async Task GetEspecificacaoDatabaseAsync(CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
        ...
        /// <summary>
        /// Método assíncrono que salva a especificação na database, inserindo um novo se o id for nulo ou editando se o id não for nulo
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task SalvarEspecificacaoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_inserir_especificacao", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_conjunto", Conjunto.Id);
                        ...
                        {
                            throw new ValorJaExisteException("especificação", Nome);
                        }
                    }
                }
                else
                {
                    // Utilização do comando
                    using (MySqlConnector.MySqlCommand command = new("sp_editar_especificacao", db.conexao))
                    {
                        // Definição do tipo do comando e dos parâmetros
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        command.Parameters.AddWithValue("p_id_especificacao", Id);
                        command.Parameters.AddWithValue("p_nome", Nome);
                        command.Parameters.AddWithValue("p_id_conjunto", Conjunto.Id);
                        command.Parameters.AddWithValue("p_id_status", Status.Id);
                        command.Parameters.Add("p_mensagem", MySqlConnector.MySqlDbType.VarChar, 255).Direction = System.Data.ParameterDirection.Output;
                        ...
                        {
                            throw new ValorNaoExisteException("especificação", Nome);
                        }
                    }
                }
            }
        ...
        /// <summary>
        /// Método assíncrono que deleta a especificação na database, desde que não exista tabelas se referenciando a esse item
        /// </summary>
        /// <param name="ct">Token de cancelamento</param>
        ...
        public async Task DeletarEspecificacaoDatabaseAsync(CancellationToken ct)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                // Utilização do comando
                using (MySqlConnector.MySqlCommand command = new("sp_excluir_especificacao", db.conexao))
                {
                    // Definição do tipo do comando e dos parâmetros
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.AddWithValue("p_id_especificacao", Id);
                    ...
                    catch (MySqlConnector.MySqlException ex)
                    {
                        ...
                        {
                            throw new ChaveEstrangeiraEmUsoException("especificação", Nome);
                        }
        ...
        /// <summary>
        /// Método assíncrono que preenche uma lista de especificações com os argumentos utilizados
        /// </summary>
        /// <param name="listaEspecificacoes">Representa a lista de especificações que deseja preencher</param>
        ...
        /// <param name="nomesParametrosSeparadosPorVirgulas">Representa o nome dos parâmetros passados nas condições extras. Devem começar com @ e ser separados por vírgulas</param>
        /// <param name="valoresParametros">Reresenta um array de parâmetros com os valores dos parâmetros definidos. Devem seguir a mesma ordem do parâmetro <paramref name="nomesParametrosSeparadosPorVirgulas"/></param>
        public static async Task PreencheListaEspecificacoesAsync(ObservableCollection<Especificacao> listaEspecificacoes, bool limparLista, IProgress<double>? reportadorProgresso, CancellationToken ct, string condicoesExtras, string nomesParametrosSeparadosPorVirgulas, params object?[] valoresParametros)
        {
            // Lança exceção de cancelamento caso ela tenha sido efetuada
            ct.ThrowIfCancellationRequested();
            ...
            {
                listaEspecificacoes = new();
            }

            // Limpa a lista caso verdadeiro
            if (limparLista)
            {
                listaEspecificacoes.Clear();
            }
            ...
            using (var db = new ConexaoMySQL())
            {
                ...
                string comando = "SELECT "
                  ...
                  + "FROM tb_especificacoes AS espe "
                  + "LEFT JOIN tb_status AS stat ON stat.id_status = espe.id_status "
                  + "LEFT JOIN tb_conjuntos AS conj ON conj.id_conjunto = espe.id_conjunto "
                  + "LEFT JOIN tb_status AS stat_conj ON stat_conj.id_status = conj.id_status "
                  + condicoesExtras;
                    ...
                    using (var reader = await command.ExecuteReaderAsync(ct))
                    {
                        ...
                        if (reader.HasRows)
                        {
                            ...
                            while (await reader.ReadAsync(ct))
                            {
                                ...
                                // Verifica se a classe não foi instânciada e, caso verdadeiro, cria nova instância da classe
                                if (item.Conjunto.Status == null)
                                {
                                    item.Conjunto.Status = new();
                                }

                                // Define as propriedades
                                item.Id = FuncoesDeConversao.ConverteParaInt(reader["IdEspecificacao"]);
                                item.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeEspecificacao"]);
                                item.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusEspecificacao"]);
                                item.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusEspecificacao"]);
                                item.Conjunto.Id = FuncoesDeConversao.ConverteParaInt(reader["IdConjunto"]);
                                item.Conjunto.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeConjunto"]);
                                item.Conjunto.Status.Id = FuncoesDeConversao.ConverteParaInt(reader["IdStatusConjunto"]);
                                item.Conjunto.Status.Nome = FuncoesDeConversao.ConverteParaString(reader["NomeStatusConjunto"]);

                                // Lança exceção de cancelamento caso ela tenha sido efetuada
                                ct.ThrowIfCancellationRequested();

                                // Adiciona o item à coleção
                                listaEspecificacoes.Add(item);
        ...
        public object Clone()
        {
            Especificacao especificacaoCopia = new();

            especificacaoCopia.Id = Id;
            especificacaoCopia.Nome = Nome;
            especificacaoCopia.Status = Status == null ? new() : (Status)Status.Clone();
            especificacaoCopia.Conjunto = Conjunto == null ? new() : (Conjunto)Conjunto.Clone();

            return especificacaoCopia;
        }
