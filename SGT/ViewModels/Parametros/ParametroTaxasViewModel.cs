...
{
    public class ParametroTaxasViewModel : ObservableObject, IPageViewModel
    {
        #region Campos

        private Taxa _taxaSelecionada = new();
        private Taxa _taxaAlterar = new();
        ...
        private ObservableCollection<Taxa> _listaTaxas = new();
        ...
        public ParametroTaxasViewModel(IDialogCoordinator dialogCoordinator)
        {
        ...
        public Taxa TaxaSelecionada
        {
            get { return _taxaSelecionada; }
            set
            {
                _taxaSelecionada = value;
                OnPropertyChanged(nameof(TaxaSelecionada));
                ...
                {
                    TaxaAlterar = (Taxa)TaxaSelecionada.Clone();
                    PermiteEditar = TaxaAlterar != null;
                    PermiteDeletar = TaxaAlterar != null;
                }
            }
        }

        public Taxa TaxaAlterar
        {
            get { return _taxaAlterar; }
            set
            {
                _taxaAlterar = value;
                OnPropertyChanged(nameof(TaxaAlterar));
            }
        }
        ...
        public ObservableCollection<Taxa> ListaTaxas
        {
            get { return _listaTaxas; }
            set
            {
                if (value != _listaTaxas)
                {
                    _listaTaxas = value;
                    OnPropertyChanged(nameof(ListaTaxas));
                }
            }
        }
        ...
        public async Task ConstrutorAsync()
        {
            try
            {
                ...
                await Taxa.PreencheListaTaxasAsync(ListaTaxas, true, null, CancellationToken.None, "ORDER BY nome ASC", "");
        ...
        private void Cancelar()
        {
            ...
            CancelarVisivel = false;
            PermiteAdicionar = true;
            PermiteEditar = TaxaAlterar != null && TaxaAlterar?.Id != null; ;
            PermiteCancelar = false;
            PermiteSalvar = false;
            PermiteDeletar = TaxaAlterar != null && TaxaAlterar?.Id != null; ;
        }
        ...
        private async Task SalvarAsync()
        {
            ...
            MensagemStatus = "Salvando dados da taxa '" + TaxaAlterar.Nome + "', aguarde...";
            CancelarVisivel = true;
            PermiteCancelar = true;
            PermiteSalvar = false;
            PermiteAdicionar = false;
            ...
            try
            {
                await TaxaAlterar.SalvarTaxaDatabaseAsync(_cts.Token);
            }
            catch (Exception ex)
            {
                ValorProgresso = 0;
            ...
            int? idTaxaAlterada = TaxaAlterar.Id;
            ...
            try
            {
                TaxaSelecionada = ListaTaxas.First(taxa => taxa.Id == idTaxaAlterada);
            }
            catch (Exception)
            {
            }
        ...
        private async Task Deletar()
        {
            ...
            var respostaMensagem = await _dialogCoordinator.ShowMessageAsync(this,
                "Atenção", "Tem certeza que deseja excluir a taxa '" + TaxaAlterar.Nome + "'? " +
            ...
            ProgressoVisivel = true;
            ProgressoEhIndeterminavel = true;
            MensagemStatus = "Deletando taxa '" + TaxaAlterar.Nome + "', aguarde...";
            CancelarVisivel = true;
            PermiteCancelar = true;
            PermiteSalvar = false;
            PermiteAdicionar = false;
            ...
            try
            {
                await TaxaAlterar.DeletarTaxaDatabaseAsync(_cts.Token);
            }
            catch (Exception ex)
            {
                ValorProgresso = 0;
