...
{
    public class ParametroAreasViewModel : ObservableObject, IPageViewModel
    {
        #region Campos

        private Area _areaSelecionada = new();
        private Area _areaAlterar = new();
        ...
        public ParametroAreasViewModel(IDialogCoordinator dialogCoordinator)
        {
        ...
        public Area AreaSelecionada
        {
            ...
            set
            {
                ...
                {
                    AreaAlterar = (Area)AreaSelecionada.Clone();
                    PermiteEditar = AreaAlterar != null;
                    PermiteDeletar = AreaAlterar != null;
                    // Define valores
                    try
                    {
                        Cliente = ListaClientes.First(clie => clie.Id == AreaSelecionada.Planta.Cliente.Id);
                    }
                    catch (Exception)
                    {
                    }
                    try
                    {
                        AreaAlterar.Planta = ListaPlantas.First(plan => plan.Id == AreaSelecionada.Planta.Id);
                    }
                    catch (Exception)
                    {
                    }
                    try
                    {
                        AreaAlterar.Status = ListaStatus.First(stat => stat.Id == AreaSelecionada.Status.Id);
                    }
                    catch (Exception)
                    {
                    }
        ...
        public Area AreaAlterar
        {
            get { return _areaAlterar; }
            set
            {
                _areaAlterar = value;
                OnPropertyChanged(nameof(AreaAlterar));
            }
        }
            ...
            set
            {
                if (value != _cliente)
                {
                    ...
                    if (ListaPlantasView.Count == 1)
                    {
                        ...
                        AreaAlterar.Planta = (Planta)ListaPlantasView.CurrentItem;
                    }
                }
            }
        }
        ...
        {
            AreaAlterar = new Area();
            Cliente = null;
            AreaAlterar.Planta = null;
            AreaAlterar.Status = null;
        ...
        private void Cancelar()
        {
            ...
            CancelarVisivel = false;
            PermiteAdicionar = true;
            PermiteEditar = AreaAlterar != null && AreaAlterar?.Id != null; ;
            PermiteCancelar = false;
            PermiteSalvar = false;
            PermiteDeletar = AreaAlterar != null && AreaAlterar?.Id != null; ;
        }
        ...
        private async Task SalvarAsync()
        {
            ...
            MensagemStatus = "Salvando dados da área '" + AreaAlterar.Nome + "', aguarde...";
            CancelarVisivel = true;
            PermiteCancelar = true;
            PermiteSalvar = false;
            PermiteAdicionar = false;
            ...
            try
            {
                await AreaAlterar.SalvarAreaDatabaseAsync(_cts.Token);
            }
            catch (Exception ex)
            {
                ValorProgresso = 0;
            ...
            int? idAreaAlterado = AreaAlterar.Id;
            ...
            try
            {
                AreaSelecionada = ListaAreas.First(area => area.Id == idAreaAlterado);
            }
            catch (Exception)
            {
            }
        ...
        private async Task Deletar()
        {
            ...
            var respostaMensagem = await _dialogCoordinator.ShowMessageAsync(this,
                "Atenção", "Tem certeza que deseja excluir a área '" + AreaAlterar.Nome + "'? " +
            ...
            ProgressoVisivel = true;
            ProgressoEhIndeterminavel = true;
            MensagemStatus = "Deletando área '" + AreaAlterar.Nome + "', aguarde...";
            CancelarVisivel = true;
            PermiteCancelar = true;
            PermiteSalvar = false;
            PermiteAdicionar = false;
            ...
            try
            {
                await AreaAlterar.DeletarAreaDatabaseAsync(_cts.Token);
            }
            catch (Exception ex)
            {
                ValorProgresso = 0;
            ...
            PermiteEditar = false;
            PermiteDeletar = false;
            MensagemStatus = "Área excluída com sucesso!";
