...
{
    public class ParametroPlantasViewModel : ObservableObject, IPageViewModel
    {
        #region Campos

        private Planta _plantaSelecionada = new();
        private Planta _plantaAlterar = new();
        ...
        public ParametroPlantasViewModel(IDialogCoordinator dialogCoordinator)
        {
        ...
        public Planta PlantaSelecionada
        {
            get { return _plantaSelecionada; }
            set
            {
                _plantaSelecionada = value;
                OnPropertyChanged(nameof(PlantaSelecionada));
                ...
                {
                    PlantaAlterar = (Planta)PlantaSelecionada.Clone();
                    PermiteEditar = PlantaAlterar != null;
                    PermiteDeletar = PlantaAlterar != null;

                    // Define valores
                    try
                    {
                        PlantaAlterar.Status = ListaStatus.First(stat => stat.Id == PlantaSelecionada.Status.Id);
                    }
                    catch (Exception)
                    {
                    }
                    try
                    {
                        PlantaAlterar.Cliente = ListaClientes.First(clie => clie.Id == PlantaSelecionada.Cliente.Id);
                    }
                    catch (Exception)
                    {
                    }
                    try
                    {
                        Pais = ListaPaises.First(pais => pais.Id == PlantaSelecionada.Cidade.Estado.Pais.Id);
                    }
                    catch (Exception)
                    {
                    }
                    try
                    {
                        Estado = ListaEstados.First(esta => esta.Id == PlantaSelecionada.Cidade.Estado.Id);
                    }
                    catch (Exception)
                    {
                    }
                    try
                    {
                        Cidade = ListaCidades.First(cida => cida.Id == PlantaSelecionada.Cidade.Id);
                    }
                    catch (Exception)
                    {
                    }
        ...
        public Planta PlantaAlterar
        {
            get { return _plantaAlterar; }
            set
            {
                _plantaAlterar = value;
                OnPropertyChanged(nameof(PlantaAlterar));
                PermiteEditar = PlantaAlterar != null && PlantaAlterar?.Id != null;
                PermiteDeletar = PlantaAlterar != null && PlantaAlterar?.Id != null;
            }
        }
        ...
        {
            PlantaAlterar = new Planta();
            PlantaAlterar.Cliente = null;
            PlantaAlterar.Status = null;
            Pais = null;
            Estado = null;
            Cidade = null;
        ...
        private async Task SalvarAsync()
        {
            ...
            MensagemStatus = "Salvando dados da planta '" + PlantaAlterar.Nome + "', aguarde...";
            CancelarVisivel = true;
            PermiteCancelar = true;
            PermiteSalvar = false;
            PermiteAdicionar = false;
            ...
            PlantaAlterar.Cidade = Cidade;

            try
            {
                await PlantaAlterar.SalvarPlantaDatabaseAsync(_cts.Token);
            }
            catch (Exception ex)
            {
                ValorProgresso = 0;
            ...
            int? idPlantateAlterado = PlantaAlterar.Id;
            ...
            try
            {
                PlantaSelecionada = ListaPlantas.First(cont => cont.Id == idPlantateAlterado);
            }
            catch (Exception)
            {
            }
        ...
        private async Task Deletar()
        {
            ...
            var respostaMensagem = await _dialogCoordinator.ShowMessageAsync(this,
                "Atenção", "Tem certeza que deseja excluir a planta '" + PlantaAlterar.Nome + "'? " +
            ...
            ProgressoVisivel = true;
            ProgressoEhIndeterminavel = true;
            MensagemStatus = "Deletando planta '" + PlantaAlterar.Nome + "', aguarde...";
            CancelarVisivel = true;
            PermiteCancelar = true;
            PermiteSalvar = false;
            PermiteAdicionar = false;
            ...
            try
            {
                await PlantaAlterar.DeletarPlantaDatabaseAsync(_cts.Token);
            }
            catch (Exception ex)
            {
                ValorProgresso = 0;
            ...
            PermiteEditar = false;
            PermiteDeletar = false;
            MensagemStatus = "Planta excluída com sucesso!";
