...
{
    public class ParametroModelosViewModel : ObservableObject, IPageViewModel
    {
        #region Campos

        private Modelo _modeloSelecionado = new();
        private Modelo _modeloAlterar = new();
        ...
        public ParametroModelosViewModel(IDialogCoordinator dialogCoordinator)
        {
        ...
        public Modelo ModeloSelecionado
        {
            ...
            set
            {
                ...
                {
                    ModeloAlterar = (Modelo)ModeloSelecionado.Clone();
                    PermiteEditar = ModeloAlterar != null;
                    PermiteDeletar = ModeloAlterar != null;
                    // Define valores
                    try
                    {
                        ModeloAlterar.Fabricante = ListaFabricantes.First(fabr => fabr.Id == ModeloSelecionado.Fabricante.Id);
                    }
                    catch (Exception)
                    {
                    }
                    try
                    {
                        ModeloAlterar.Categoria = ListaCategorias.First(cate => cate.Id == ModeloSelecionado.Categoria.Id);
                    }
                    catch (Exception)
                    {
                    }
                    try
                    {
                        ModeloAlterar.TipoEquipamento = ListaTiposEquipamentos.First(tieq => tieq.Id == ModeloSelecionado.TipoEquipamento.Id);
                    }
                    catch (Exception)
                    {
                    }
                    try
                    {
                        ModeloAlterar.Classe = ListaClasses.First(clas => clas.Id == ModeloSelecionado.Classe.Id);
                    }
                    catch (Exception)
                    {
                    }
                    try
                    {
                        ModeloAlterar.Status = ListaStatus.First(stat => stat.Id == ModeloSelecionado.Status.Id);
                    }
                    catch (Exception)
                    {
                    }
        ...
        public Modelo ModeloAlterar
        {
            get { return _modeloAlterar; }
            set
            {
                _modeloAlterar = value;
                OnPropertyChanged(nameof(ModeloAlterar));
            }
        }
        ...
        {
            ModeloAlterar = new Modelo();
            ModeloAlterar.Status = null;
            ModeloAlterar.Fabricante = null;
            ModeloAlterar.Categoria = null;
            ModeloAlterar.TipoEquipamento = null;
            ModeloAlterar.Classe = null;
        ...
        private void Cancelar()
        {
            ...
            CancelarVisivel = false;
            PermiteAdicionar = true;
            PermiteEditar = ModeloAlterar != null && ModeloAlterar?.Id != null; ;
            PermiteCancelar = false;
            PermiteSalvar = false;
            PermiteDeletar = ModeloAlterar != null && ModeloAlterar?.Id != null; ;
        }
        ...
        private async Task SalvarAsync()
        {
            ...
            MensagemStatus = "Salvando dados do modelo '" + ModeloAlterar.Nome + "', aguarde...";
            CancelarVisivel = true;
            PermiteCancelar = true;
            PermiteSalvar = false;
            PermiteAdicionar = false;
            ...
            try
            {
                await ModeloAlterar.SalvarModeloDatabaseAsync(_cts.Token);
            }
            catch (Exception ex)
            {
                ValorProgresso = 0;
            ...
            int? idModeloAlterado = ModeloAlterar.Id;
            ...
            try
            {
                ModeloSelecionado = ListaModelos.First(mode => mode.Id == idModeloAlterado);
            }
            catch (Exception)
            {
            }
        ...
        private async Task Deletar()
        {
            ...
            var respostaMensagem = await _dialogCoordinator.ShowMessageAsync(this,
                "Atenção", "Tem certeza que deseja excluir o modelo '" + ModeloAlterar.Nome + "'? " +
            ...
            ProgressoVisivel = true;
            ProgressoEhIndeterminavel = true;
            MensagemStatus = "Deletando modelo '" + ModeloAlterar.Nome + "', aguarde...";
            CancelarVisivel = true;
            PermiteCancelar = true;
            PermiteSalvar = false;
            PermiteAdicionar = false;
            ...
            try
            {
                await ModeloAlterar.DeletarModeloDatabaseAsync(_cts.Token);
            }
            catch (Exception ex)
            {
                ValorProgresso = 0;
            ...
            PermiteEditar = false;
            PermiteDeletar = false;
            MensagemStatus = "Modelo excluído com sucesso!";
