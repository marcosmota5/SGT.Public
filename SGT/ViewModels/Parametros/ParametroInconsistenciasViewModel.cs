...
{
    public class ParametroInconsistenciasViewModel : ObservableObject, IPageViewModel
    {
        #region Campos

        private Inconsistencia _inconsistenciaSelecionada = new();
        private Inconsistencia _inconsistenciaAlterar = new();
        ...
        public ParametroInconsistenciasViewModel(IDialogCoordinator dialogCoordinator)
        {
        ...
        public void LimparViewModel()
        {
            try
            {
                _inconsistenciaSelecionada = null;
                _cts = null;
        ...
        public Inconsistencia InconsistenciaSelecionada
        {
            get { return _inconsistenciaSelecionada; }
            set
            {
                _inconsistenciaSelecionada = value;
                OnPropertyChanged(nameof(InconsistenciaSelecionada));
                if (InconsistenciaSelecionada == null)
                {
                    InconsistenciaAlterar = null;
                }
                else
                {
                    InconsistenciaAlterar = (Inconsistencia)InconsistenciaSelecionada.Clone();
                    PermiteEditar = InconsistenciaAlterar != null;
                    PermiteDeletar = InconsistenciaAlterar != null;
                    // Define valores
                    try
                    {
                        InconsistenciaAlterar.Status = ListaStatus.First(stat => stat.Id == InconsistenciaSelecionada.Status.Id);
                    }
                    catch (Exception)
                    {
                    }
        ...
        public Inconsistencia InconsistenciaAlterar
        {
            get { return _inconsistenciaAlterar; }
            set
            {
                _inconsistenciaAlterar = value;
                OnPropertyChanged(nameof(InconsistenciaAlterar));
            }
        }
        ...
        public async Task ConstrutorAsync()
        {
            try
            {
                ...
                await Inconsistencia.PreencheListaInconsistenciasAsync(ListaInconsistencias, true, null, CancellationToken.None, "ORDER BY inco.nome ASC", "");
        ...
        {
            InconsistenciaAlterar = new Inconsistencia();
            InconsistenciaAlterar.Status = null;
        ...
        private void Cancelar()
        {
            ...
            CancelarVisivel = false;
            PermiteAdicionar = true;
            PermiteEditar = InconsistenciaAlterar != null && InconsistenciaAlterar?.Id != null; ;
            PermiteCancelar = false;
            PermiteSalvar = false;
            PermiteDeletar = InconsistenciaAlterar != null && InconsistenciaAlterar?.Id != null; ;
        }
        ...
        private async Task SalvarAsync()
        {
            ...
            MensagemStatus = "Salvando dados da inconsistência '" + InconsistenciaAlterar.Nome + "', aguarde...";
            CancelarVisivel = true;
            PermiteCancelar = true;
            PermiteSalvar = false;
            PermiteAdicionar = false;
            ...
            try
            {
                await InconsistenciaAlterar.SalvarInconsistenciaDatabaseAsync(_cts.Token);
            }
            catch (Exception ex)
            {
                ValorProgresso = 0;
            ...
            int? idInconsistenciaAlterado = InconsistenciaAlterar.Id;
            ...
            try
            {
                InconsistenciaSelecionada = ListaInconsistencias.First(inco => inco.Id == idInconsistenciaAlterado);
            }
            catch (Exception)
            {
            }
        ...
        private async Task Deletar()
        {
            ...
            var respostaMensagem = await _dialogCoordinator.ShowMessageAsync(this,
                "Atenção", "Tem certeza que deseja excluir a inconsistência '" + InconsistenciaAlterar.Nome + "'? " +
            ...
            ProgressoVisivel = true;
            ProgressoEhIndeterminavel = true;
            MensagemStatus = "Deletando inconsistência '" + InconsistenciaAlterar.Nome + "', aguarde...";
            CancelarVisivel = true;
            PermiteCancelar = true;
            PermiteSalvar = false;
            PermiteAdicionar = false;
            ...
            try
            {
                await InconsistenciaAlterar.DeletarInconsistenciaDatabaseAsync(_cts.Token);
            }
            catch (Exception ex)
            {
                ValorProgresso = 0;
            ...
            PermiteEditar = false;
            PermiteDeletar = false;
            MensagemStatus = "Inconsistência excluída com sucesso!";
