...
{
    public class ParametroFrotasViewModel : ObservableObject, IPageViewModel
    {
        #region Campos

        private Frota _frotaSelecionada = new();
        private Frota _frotaAlterar = new();
        ...
        public ParametroFrotasViewModel(IDialogCoordinator dialogCoordinator)
        {
        ...
        {
            get { return _frotaSelecionada; }
            set
            {
                _frotaSelecionada = value;
                OnPropertyChanged(nameof(FrotaSelecionada));
                ...
                {
                    FrotaAlterar = (Frota)FrotaSelecionada.Clone();
                    PermiteEditar = FrotaAlterar != null;
                    PermiteDeletar = FrotaAlterar != null;

                    // Define valores
                    try
                    {
                        Cliente = ListaClientes.First(clie => clie.Id == FrotaSelecionada.Area.Planta.Cliente.Id);
                    }
                    catch (Exception)
                    {
                    }
                    try
                    {
                        Planta = ListaPlantas.First(plan => plan.Id == FrotaSelecionada.Area.Planta.Id);
                    }
                    catch (Exception)
                    {
                    }
                    try
                    {
                        Area = ListaAreas.First(area => area.Id == FrotaSelecionada.Area.Id);
                    }
                    catch (Exception)
                    {
                    }
                    try
                    {
                        FrotaAlterar.Status = ListaStatus.First(stat => stat.Id == FrotaSelecionada.Status.Id);
                    }
                    catch (Exception)
                    {
                    }
        ...
        public Frota FrotaAlterar
        {
            get { return _frotaAlterar; }
            set
            {
                _frotaAlterar = value;
                OnPropertyChanged(nameof(FrotaAlterar));
                PermiteEditar = FrotaAlterar != null && FrotaAlterar?.Id != null;
                PermiteDeletar = FrotaAlterar != null && FrotaAlterar?.Id != null;
            }
        }
        ...
        {
            FrotaAlterar = new Frota();
            Cliente = null;
            FrotaAlterar.Status = null;
            Planta = null;
            Area = null;
        ...
        private async Task SalvarAsync()
        {
            ...
            MensagemStatus = "Salvando dados da frota '" + FrotaAlterar.Nome + "', aguarde...";
            CancelarVisivel = true;
            PermiteCancelar = true;
            PermiteSalvar = false;
            PermiteAdicionar = false;
            ...
            FrotaAlterar.Area = Area;

            try
            {
                await FrotaAlterar.SalvarFrotaDatabaseAsync(_cts.Token);
            }
            catch (Exception ex)
            {
                ValorProgresso = 0;
            ...
            int? idFrotateAlterado = FrotaAlterar.Id;
            ...
            try
            {
                FrotaSelecionada = ListaFrotas.First(frot => frot.Id == idFrotateAlterado);
            }
            catch (Exception)
            {
            }
        ...
        private async Task Deletar()
        {
            ...
            var respostaMensagem = await _dialogCoordinator.ShowMessageAsync(this,
                "Atenção", "Tem certeza que deseja excluir a frota '" + FrotaAlterar.Nome + "'? " +
            ...
            ProgressoVisivel = true;
            ProgressoEhIndeterminavel = true;
            MensagemStatus = "Deletando frota '" + FrotaAlterar.Nome + "', aguarde...";
            CancelarVisivel = true;
            PermiteCancelar = true;
            PermiteSalvar = false;
            PermiteAdicionar = false;
            ...
            try
            {
                await FrotaAlterar.DeletarFrotaDatabaseAsync(_cts.Token);
            }
            catch (Exception ex)
            {
                ValorProgresso = 0;
            ...
            PermiteEditar = false;
            PermiteDeletar = false;
            MensagemStatus = "Frota excluída com sucesso!";
