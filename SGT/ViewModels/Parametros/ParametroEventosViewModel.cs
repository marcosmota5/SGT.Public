...
{
    public class ParametroEventosViewModel : ObservableObject, IPageViewModel
    {
        #region Campos

        private Evento _eventoSelecionado = new();
        private Evento _eventoAlterar = new();
        ...
        public ParametroEventosViewModel(IDialogCoordinator dialogCoordinator)
        {
        ...
        public Evento EventoSelecionado
        {
            ...
            set
            {
                ...
                {
                    EventoAlterar = (Evento)EventoSelecionado.Clone();
                    PermiteEditar = EventoAlterar != null;
                    PermiteDeletar = EventoAlterar != null;
                    // Define valores
                    try
                    {
                        EventoAlterar.Status = ListaStatus.First(stat => stat.Id == EventoSelecionado.Status.Id);
                    }
                    catch (Exception)
                    {
                    }
        ...
        public Evento EventoAlterar
        {
            get { return _eventoAlterar; }
            set
            {
                _eventoAlterar = value;
                OnPropertyChanged(nameof(EventoAlterar));
            }
        }
        ...
        public async Task ConstrutorAsync()
        {
            try
            {
                ...
                await Evento.PreencheListaEventosAsync(ListaEventos, true, null, CancellationToken.None, "ORDER BY even.nome ASC", "");
        ...
        {
            EventoAlterar = new Evento();
            EventoAlterar.Status = null;
        ...
        private void Cancelar()
        {
            ...
            CancelarVisivel = false;
            PermiteAdicionar = true;
            PermiteEditar = EventoAlterar != null && EventoAlterar?.Id != null; ;
            PermiteCancelar = false;
            PermiteSalvar = false;
            PermiteDeletar = EventoAlterar != null && EventoAlterar?.Id != null; ;
        }
        ...
        private async Task SalvarAsync()
        {
            ...
            MensagemStatus = "Salvando dados do evento '" + EventoAlterar.Nome + "', aguarde...";
            CancelarVisivel = true;
            PermiteCancelar = true;
            PermiteSalvar = false;
            PermiteAdicionar = false;
            ...
            try
            {
                await EventoAlterar.SalvarEventoDatabaseAsync(_cts.Token);
            }
            catch (Exception ex)
            {
                ValorProgresso = 0;
            ...
            int? idEventoAlterado = EventoAlterar.Id;
            ...
            try
            {
                EventoSelecionado = ListaEventos.First(even => even.Id == idEventoAlterado);
            }
            catch (Exception)
            {
            }
        ...
        private async Task Deletar()
        {
            ...
            var respostaMensagem = await _dialogCoordinator.ShowMessageAsync(this,
                "Atenção", "Tem certeza que deseja excluir o evento '" + EventoAlterar.Nome + "'? " +
            ...
            ProgressoVisivel = true;
            ProgressoEhIndeterminavel = true;
            MensagemStatus = "Deletando evento '" + EventoAlterar.Nome + "', aguarde...";
            CancelarVisivel = true;
            PermiteCancelar = true;
            PermiteSalvar = false;
            PermiteAdicionar = false;
            ...
            try
            {
                await EventoAlterar.DeletarEventoDatabaseAsync(_cts.Token);
            }
            catch (Exception ex)
            {
                ValorProgresso = 0;
            ...
            PermiteEditar = false;
            PermiteDeletar = false;
            MensagemStatus = "Evento excluído com sucesso!";
