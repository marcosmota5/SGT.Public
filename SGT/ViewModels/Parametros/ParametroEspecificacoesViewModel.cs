...
{
    public class ParametroEspecificacoesViewModel : ObservableObject, IPageViewModel
    {
        #region Campos

        private Especificacao _especificacaoSelecionada = new();
        private Especificacao _especificacaoAlterar = new();
        ...
        public ParametroEspecificacoesViewModel(IDialogCoordinator dialogCoordinator)
        {
        ...
        public void LimparViewModel()
        {
            try
            {
                _especificacaoSelecionada = null;
                _cts = null;
        ...
        public Especificacao EspecificacaoSelecionada
        {
            get { return _especificacaoSelecionada; }
            set
            {
                _especificacaoSelecionada = value;
                OnPropertyChanged(nameof(EspecificacaoSelecionada));
                if (EspecificacaoSelecionada == null)
                {
                    EspecificacaoAlterar = null;
                }
                else
                {
                    EspecificacaoAlterar = (Especificacao)EspecificacaoSelecionada.Clone();
                    PermiteEditar = EspecificacaoAlterar != null;
                    PermiteDeletar = EspecificacaoAlterar != null;
                    // Define valores
                    try
                    {
                        EspecificacaoAlterar.Conjunto = ListaConjuntos.First(conj => conj.Id == EspecificacaoSelecionada.Conjunto.Id);
                    }
                    catch (Exception)
                    {
                    }
                    try
                    {
                        EspecificacaoAlterar.Status = ListaStatus.First(stat => stat.Id == EspecificacaoSelecionada.Status.Id);
                    }
                    catch (Exception)
                    {
                    }
        ...
        public Especificacao EspecificacaoAlterar
        {
            get { return _especificacaoAlterar; }
            set
            {
                _especificacaoAlterar = value;
                OnPropertyChanged(nameof(EspecificacaoAlterar));
            }
        }
        ...
        public async Task ConstrutorAsync()
        {
            try
            {
                ...
                await Especificacao.PreencheListaEspecificacoesAsync(ListaEspecificacoes, true, null, CancellationToken.None, "ORDER BY espe.nome ASC", "");
                await Conjunto.PreencheListaConjuntosAsync(ListaConjuntos, true, null, CancellationToken.None, "ORDER BY conj.nome ASC", "");
        ...
        {
            EspecificacaoAlterar = new Especificacao();
            EspecificacaoAlterar.Conjunto = null;
            EspecificacaoAlterar.Status = null;
        ...
        private void Cancelar()
        {
            ...
            CancelarVisivel = false;
            PermiteAdicionar = true;
            PermiteEditar = EspecificacaoAlterar != null && EspecificacaoAlterar?.Id != null; ;
            PermiteCancelar = false;
            PermiteSalvar = false;
            PermiteDeletar = EspecificacaoAlterar != null && EspecificacaoAlterar?.Id != null; ;
        }
        ...
        private async Task SalvarAsync()
        {
            ...
            MensagemStatus = "Salvando dados da especificação '" + EspecificacaoAlterar.Nome + "', aguarde...";
            CancelarVisivel = true;
            PermiteCancelar = true;
            PermiteSalvar = false;
            PermiteAdicionar = false;
            ...
            try
            {
                await EspecificacaoAlterar.SalvarEspecificacaoDatabaseAsync(_cts.Token);
            }
            catch (Exception ex)
            {
                ValorProgresso = 0;
            ...
            int? idEspecificacaoAlterado = EspecificacaoAlterar.Id;
            ...
            try
            {
                EspecificacaoSelecionada = ListaEspecificacoes.First(espe => espe.Id == idEspecificacaoAlterado);
            }
            catch (Exception)
            {
            }
        ...
        private async Task Deletar()
        {
            ...
            var respostaMensagem = await _dialogCoordinator.ShowMessageAsync(this,
                "Atenção", "Tem certeza que deseja excluir a especificação '" + EspecificacaoAlterar.Nome + "'? " +
            ...
            ProgressoVisivel = true;
            ProgressoEhIndeterminavel = true;
            MensagemStatus = "Deletando especificação '" + EspecificacaoAlterar.Nome + "', aguarde...";
            CancelarVisivel = true;
            PermiteCancelar = true;
            PermiteSalvar = false;
            PermiteAdicionar = false;
            ...
            try
            {
                await EspecificacaoAlterar.DeletarEspecificacaoDatabaseAsync(_cts.Token);
            }
            catch (Exception ex)
            {
                ValorProgresso = 0;
            ...
            PermiteEditar = false;
            PermiteDeletar = false;
            MensagemStatus = "Especificação excluída com sucesso!";
